    ...
    <div class="container-fluid">
        ...
        <div class="row justify-content-center">
            ...
            <div class="col-12 col-md-8">
                <div class="card">
                    <div class="card-body">
                        <form action="/{{ auth()->user()->status->route() }}/pengajuan-izin/{{ $pengajuan['id'] }}"
                            method="POST">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                                ...
                                <select name="jenis_izin" class="form-control" id="jenis_izin" required>
                                    ...
                                    @foreach ($jenisIzin as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == old('jenis_izin', $pengajuan['jenis_izin']) ? 'selected' : '' }}>
                                            {{ $item->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="keterangan" id="keterangan" rows="1" class="form-control" required>{{ $pengajuan['keterangan'] }}</textarea>
                            </div>
