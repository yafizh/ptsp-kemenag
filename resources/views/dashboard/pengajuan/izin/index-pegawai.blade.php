    ...
    <div class="container-fluid">
        <div class="row mb-3">
            ...
            <div class="col-12 col-md-6 d-flex justify-content-end">
                <a href="/{{ auth()->user()->status->route() }}/pengajuan-izin/create" class="btn btn-primary">
                    Pengajuan Baru
                </a>
            </div>
        </div>
                            ...
                            <tbody>
                                @foreach ($pengajuan as $item)
                                    <tr>
                                        ...
                                        <td class="text-center align-middle">{{ $item['jenis_izin'] }}</td>
                                        ...
                                        <td class="td-fit align-middle">
                                            <div class="d-flex gap-1 justify-content-center">
                                                <a href="/{{ auth()->user()->status->route() }}/pengajuan-izin/{{ $item['id'] }}"
                                                    class="btn btn-sm btn-info">
                                                    Detail
                                                </a>
                                                @if (is_null($item['status']))
                                                    <a href="/{{ auth()->user()->status->route() }}/pengajuan-izin/{{ $item['id'] }}/edit"
                                                        class="btn btn-sm btn-warning">
                                                        Edit
                                                    </a>
                                                    <form
                                                        action="/{{ auth()->user()->status->route() }}/pengajuan-izin/{{ $item['id'] }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
