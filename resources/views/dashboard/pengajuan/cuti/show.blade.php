                ...
                <div class="card">
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" value="{{ $pengajuan['jenis_cuti'] }}" disabled>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea @disabled(!is_null($pengajuan['status'])) name="keterangan" id="keterangan" class="form-control">{{ $pengajuan['keterangan'] }}</textarea>
                        </div>
                        @if (is_null($pengajuan['status']))
                            <div class="d-flex justify-content-end gap-1">
                                <form
                                    action="/{{ auth()->user()->status->route() }}/pengajuan-cuti/{{ $pengajuan['id'] }}/tolak"
                                    method="POST">
                                    @csrf
                                ...
                                <form
                                    action="/{{ auth()->user()->status->route() }}/pengajuan-cuti/{{ $pengajuan['id'] }}/terima"
                                    method="POST">
                                    @csrf
