...
class PermohonanRisetController extends Controller
{
    public function index()
    {
        $permohonan = Permohonan::with(['pemohon', 'riset', 'permohonanTerverifikasi'])
            ->whereHas('riset')
        ...
        return view('dashboard.permohonan.riset.index', compact('permohonan'));
    }
    ...
    public function show(Permohonan $permohonan)
    {
        $permohonan = [
            ...
            'asal_instansi'             => $permohonan->riset->asal_instansi,
            'keperluan'                 => $permohonan->riset->keperluan,
            'tanggal_mulai'             => $permohonan->riset->tanggalMulaiFormatIndonesia(),
            'tanggal_selesai'           => $permohonan->riset->tanggalSelesaiFormatIndonesia(),
        ...
        return view('dashboard.permohonan.riset.show', compact('permohonan'));
    }
    ...
    public function terima(Request $request, Permohonan $permohonan)
    {
        ...
        return redirect('/' . auth()->user()->status->route() . '/permohonan-riset/' . $permohonan->id)->with('success', 'Berhasil menerima permohonan.');
    }
    ...
    public function tolak(Request $request, Permohonan $permohonan)
    {
        ...
        return redirect('/' . auth()->user()->status->route() . '/permohonan-riset/' . $permohonan->id)->with('success', 'Berhasil menolak permohonan.');
    }
        ...
        $permohonan = $permohonan->get()
            ->map(function ($item) {
                return [
                    ...
                    'nama'               => $item->pemohon->nama,
                    'nomor_telepon'      => $item->pemohon->nomor_telepon,
                    'asal_instansi'      => $item->riset->asal_instansi,
                    'keperluan'          => $item->riset->keperluan,
                    'status'             => is_null($item->permohonanTerverifikasi) ? null : $item->permohonanTerverifikasi->status
                ];
            });

        if ($request->get('print'))
            return view('dashboard.permohonan.riset.cetak', compact('permohonan', 'filter'));

        return view('dashboard.permohonan.riset.laporan', compact('permohonan', 'filter'));
    }
}
